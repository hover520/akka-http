...
import part2_lowlevelserver_my.GuitarDB.{AddQuantity, CreateGuitar, FinaGuitarsInStock, FindAllGuitars, FindGuitar, GuitarCreated}
...
/**
  * @author : HoverKan
  * @version : 1.0.0 :: 2022-05-04 17:49
  */
...
object GuitarDB {
  ...
  case class FinaGuitarsInStock(inStock:Boolean)
}
...
class GuitarDB extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    ...
    case AddQuantity(id,quantity) =>
      log.info(s"Trying to and $quantity items for guitar $id")
    ...
    case FinaGuitarsInStock(inStock) =>
      log.info(s"Searching for all guitar ${if (inStock) "in" else "out of"} stock")
      if (inStock)
...
object LowLevelRest extends App with GuitarStoreJsonProtocol {
  ...
  val requestHandler: HttpRequest => Future[HttpResponse] = {
    ...
    case HttpRequest(HttpMethods.GET,uri@Uri.Path("/api/guitar/inventory"),_,_,_) =>
      ...
      println(inStockOption)
      // TODO send a message to the guitarDB to fetch all guitars in/out of stock
      inStockOption match {
        case Some(inStock) =>
          val guitarsFuture: Future[List[Guitar]] = (guitarDb ? FinaGuitarsInStock(inStock)).mapTo[List[Guitar]]
  ...
  /**
    * Exercise: enhance the Guitar case class with a quantity field , bu default 0
